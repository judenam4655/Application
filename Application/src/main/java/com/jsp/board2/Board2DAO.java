...
public class Board2DAO {
	...
	// Generate Board2DAO Object
	private static Board2DAO instance = new Board2DAO();
	
	// Return static Board2DAO Object
	...
	// return ArrayList of Board2DTO Object
	public ArrayList<Board2DTO> list() {
		
		ArrayList<Board2DTO> list = new ArrayList<Board2DTO>();
		
		String query = "select * from board2 ORDER BY bGroup DESC, bStep ASC";
		// Group : bId 값
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new Board2DTO(bId, bWriter, bTitle, bContent, bDate, bHit, bGroup, bStep, bIndent));
			}
	...
	public int insert(String bWirter, String bTitle, String bContent) {
		...
		try {
			conn = ds.getConnection();
			String query = "insert into board2 (bId, bWriter, bTitle, bContent, "
					+ "bHit, bGroup, bStep, bIndent) values "
					+ "(board2_seq.nextval, ?, ?, ?, 0, board2_seq.currval, 0, 0 )";
	...
	public Board2DTO contentView(String strID) {
		...
		try {
			...
			String query = "select * from board2 where bId = ?";
			pstmt = conn.prepareStatement(query);
			...
			if(rs.next()) {
				...
				dto = new Board2DTO(bId, bWriter, bTitle, bContent, 
	...
	private void upHit(String bId) {
		...
		try {
			conn = ds.getConnection();
			String query = "update board2 set bHit = bHit + 1 where bId = ?";
			pstmt = conn.prepareStatement(query);
	...
	public int modify(String bId, String bWriter, String bTitle, String bContent) {
		...
		try {
			...
			String query = "update board2 set bWriter = ?, bTitle = ?, bContent = ? where bId = ?";
			pstmt = conn.prepareStatement(query);
	...
	public int delete(String bId) {
		...
		try {
			...
			conn = ds.getConnection();
			String query = "delete from board2 where bId = ?";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, Integer.parseInt(bId));
			result = pstmt.executeUpdate();
	...
	public Board2DTO replyView(String str) {
		Board2DTO dto = null;
		...
		try {
			...
			if(rs.next()) {
				...
				dto = new Board2DTO(bId, bWriter, bTitle, bContent, bDate, bHit, bGroup, bStep, bIndent);
			}
	...
	public int reply(String bWriter, String bTitle, String bContent, String bGroup, String bStep, String bIndent) {
		...
		try {
			conn = ds.getConnection();
			String query = "insert into board2 (bId, bWriter, bTitle, bContent, "
					+ "bGroup, bStep, bIndent) values (board2_seq.nextval, ?, ?, ?, ?, ?, ?)";
			pstmt = conn.prepareStatement(query);
	...
	private void replyShape(String strGroup, String strStep) {
		...
		try {
			conn = ds.getConnection();
			String query = "update board2 set bStep = bStep + 1 "
